...
if [ -z "$SUPABASE_URL" ] || [ -z "$SUPABASE_SERVICE_ROLE_KEY" ]; then
  ...
  # 🌟 STEP 1: Export the exact variable the CLI reads natively to replace 'supabase link' completely!
  export SUPABASE_PROJECT_ID="${REF}"

  export SUPABASE_DB_PASSWORD="$SUPABASE_DB_PASSWORD"
  
  # DB_POOLER_URL="postgresql://postgres.${REF}:${SUPABASE_DB_PASSWORD}@aws-1-us-east-1.pooler.supabase.com:6543/postgres"
