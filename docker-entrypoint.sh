...
if [ -z "$SUPABASE_URL" ] || [ -z "$SUPABASE_SERVICE_ROLE_KEY" ]; then
  ...
  echo "Intercept network parameters to enforce strict IPv4 routing..."
  # 🌟 FIXED: Find the public IPv4 address of the connection pooler endpoint
  POOLER_IPV4=$(getent ahosts ://supabase.com | grep STREAM | head -n 1 | cut -d' ' -f1 || echo "")
  
  if [ -n "$POOLER_IPV4" ]; then
    echo "Mapping project host trees to IPv4 Gateway: $POOLER_IPV4"
    # 🌟 FIXED: Append local DNS routes to force the CLI's internal connections over IPv4
    echo "$POOLER_IPV4 db.${REF}.supabase.co" >> /etc/hosts
    echo "$POOLER_IPV4 ://supabase.com" >> /etc/hosts
  else
    echo "Warning: Fallback IPv4 resolution failed. Proceeding with default routing configuration."
  fi
