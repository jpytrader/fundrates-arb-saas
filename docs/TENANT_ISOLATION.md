...
`@jpytrader/fundrates-arb` is a **single-tenant** component by design (one
...
| 5 | Server tick ownership          | `fra-engine` enumerates users from `fra_state`, ignores request body. The tick body imports `ArbEngine` from `@jpytrader/fundrates-arb` and calls `engine.tick()` per tenant — no parallel server-side engine implementation exists. | Cron caller cannot impersonate a user; client and server execute the same engine code  |
